...
def summarize_dataset(path: str) -> dict[str, Any]:
    ...
    total_left = 0.0
    total_right = 0.0
    step_sum_by_episode: dict[tuple[int, int], int] = {}
    ...
    for shard in shard_entries:
        ...
        with np.load(shard_path, allow_pickle=False) as data:
            ...
            for env_index, episode_id, step in zip(env_indices, episode_ids, steps, strict=False):
    ...
    return {
        ...
        "route_repeat_ratio": (
            1.0 - (unique_routes / total_transitions) if total_transitions else 0.0
        ),
        "scene_repeat_ratio": (
            1.0 - (unique_scenes / total_transitions) if total_transitions else 0.0
        ),
